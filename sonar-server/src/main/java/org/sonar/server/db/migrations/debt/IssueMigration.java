...
public class IssueMigration implements DatabaseMigration {
  ...
  private static final String SQL_UPDATE_ISSUE = "UPDATE issues SET technical_debt=?,updated_at=? WHERE id=?";
  ...
  static {
    StringBuilder sb = new StringBuilder("select i.id as "+ ID +", i.technical_debt as " + DEBT +
      " from issues i " +
      " where ");
    for (int i = 0; i < IssueReferentials.GROUP_SIZE; i++) {
      if (i > 0) {
        sb.append(" or ");
      }
      sb.append("i.id=?");
  ...
  public void execute() {
    try {
      ...
      IssueReferentials referentials = new IssueReferentials(db);
      if (referentials.totalIssues() > 0) {
        logger.info("Migrate {} issues", referentials.totalIssues());
  ...
  public Object convert(IssueReferentials referentials) throws Exception {
    // For each group of 1000 issue ids:
    // - load related issues
    // - in a transaction
    //   -- update issues

    Long[] issueIds = referentials.pollGroupOfIssueIds();
    while (issueIds.length > 0) {
      List<Map<String, Object>> rows = selectRows(issueIds);
      convert(rows, issueIds);

      issueIds = referentials.pollGroupOfIssueIds();
    }
    return null;
  }
  ...
  private void convert(List<Map<String, Object>> rows, Long[] issueIds) throws SQLException {
    ...
    try {
      ...
      for (Map<String, Object> row : rows) {
        ...
        params[0] = createFromLong((Long) row.get(DEBT), hoursInDay);
        params[1] = new Date(system2.now());
        params[2] = row.get(ID);
        allParams.add(params);
      }
      runner.batch(writeConnection, SQL_UPDATE_ISSUE, allParams.toArray(new Object[allParams.size()][]));
  ...
  static long createFromLong(long durationInLong, int hoursInDay) {
