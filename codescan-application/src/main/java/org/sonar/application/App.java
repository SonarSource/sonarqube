...
public class App {
    ...
    public void start(String[] cliArguments) throws IOException {
        ...
        try (AppState appState = new AppStateFactory(settings).create()) {
            ...
            CommandFactory commandFactory = new CommandFactoryImpl(settings.getProps(), fileSystem.getTempDir(),
                    System2.INSTANCE, JavaVersion.INSTANCE);

            try (ProcessLauncher processLauncher = new ProcessLauncherImpl(fileSystem.getTempDir())) {
                Scheduler scheduler = new CodeScanSchedulerImpl(settings, appReloader, commandFactory, processLauncher,
                        appState);
    ...
    private final class ShutdownHook extends Thread {
        ...
        private ShutdownHook(Scheduler scheduler) {
            super("Shutdown Hook");
