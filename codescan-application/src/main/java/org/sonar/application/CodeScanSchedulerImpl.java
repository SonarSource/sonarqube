...
public class CodeScanSchedulerImpl implements Scheduler, ManagedProcessEventListener, ProcessLifecycleListener,
        AppStateListener {
    ...
    public void schedule() throws InterruptedException {
        ...
        for (ProcessId processId : ClusterSettings.getEnabledProcesses(settings)) {
            ManagedProcessHandler process = ManagedProcessHandler.builder(processId)
                    ...
                    .setWatcherDelayMs(ManagedProcessHandler.DEFAULT_WATCHER_DELAY_MS)
    ...
    public void onAppStateOperational(ProcessId processId) {
        if (nodeLifecycle.getState() == NodeLifecycle.State.STARTING) {
            try {
                ...
                LOG.debug("Startup process was interrupted on notification that process [{}] was operation",
                        processId.getKey(), e);
                hardStopAsync();
                Thread.currentThread().interrupt();
            }
    ...
    private final class RestarterThread extends Thread {
    ...
    private final class RestartStopperThread extends Thread {
    ...
    private final class HardStopperThread extends Thread {
