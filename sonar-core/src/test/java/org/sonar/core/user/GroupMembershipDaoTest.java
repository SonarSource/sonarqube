...
public class GroupMembershipDaoTest {
  ...
  public void count_groups() throws Exception {
    ...
    // 200 is member of 3 groups
    assertThat(dao.countGroups(GroupMembershipQuery.builder().login("arthur").membership(GroupMembershipQuery.IN).build(), 200L)).isEqualTo(3);
    assertThat(dao.countGroups(GroupMembershipQuery.builder().login("arthur").membership(GroupMembershipQuery.OUT).build(), 200L)).isZero();
    // 201 is member of 1 group on 3
    assertThat(dao.countGroups(GroupMembershipQuery.builder().login("arthur").membership(GroupMembershipQuery.IN).build(), 201L)).isEqualTo(1);
    assertThat(dao.countGroups(GroupMembershipQuery.builder().login("arthur").membership(GroupMembershipQuery.OUT).build(), 201L)).isEqualTo(2);
    // 999 is member of 0 group
    assertThat(dao.countGroups(GroupMembershipQuery.builder().login("arthur").membership(GroupMembershipQuery.IN).build(), 999L)).isZero();
    assertThat(dao.countGroups(GroupMembershipQuery.builder().login("arthur").membership(GroupMembershipQuery.OUT).build(), 2999L)).isEqualTo(3);
  }
}
