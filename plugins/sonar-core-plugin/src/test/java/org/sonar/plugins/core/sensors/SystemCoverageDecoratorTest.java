...
public class SystemCoverageDecoratorTest {
  private final SystemCoverageDecorator decorator = new SystemCoverageDecorator();
  ...
  public void should_use_metrics() {
    ...
    assertThat(metrics).containsOnly(CoreMetrics.SYSTEM_LINES_TO_COVER, CoreMetrics.SYSTEM_UNCOVERED_LINES, CoreMetrics.NEW_SYSTEM_LINES_TO_COVER,
        CoreMetrics.NEW_SYSTEM_UNCOVERED_LINES, CoreMetrics.SYSTEM_CONDITIONS_TO_COVER, CoreMetrics.SYSTEM_UNCOVERED_CONDITIONS,
        CoreMetrics.NEW_SYSTEM_CONDITIONS_TO_COVER, CoreMetrics.NEW_SYSTEM_UNCOVERED_CONDITIONS);
  }
  ...
  public void coverage() {
    ...
    verify(context).saveMeasure(CoreMetrics.SYSTEM_COVERAGE, 20.0);
  }
  ...
  public void coverageCanBe0() {
    ...
    verify(context).saveMeasure(CoreMetrics.SYSTEM_COVERAGE, 0.0);
  }
  ...
  public void coverageCanBe100() {
    ...
    verify(context).saveMeasure(CoreMetrics.SYSTEM_COVERAGE, 100.0);
  }
  ...
  public void noCoverageIfNoElements() {
    ...
    verify(context, never()).saveMeasure(eq(CoreMetrics.SYSTEM_COVERAGE), anyDouble());
  }
  ...
  private static DecoratorContext mockContext(int lines, int uncoveredLines, int conditions, int uncoveredConditions) {
    ...
    when(context.getMeasure(CoreMetrics.SYSTEM_CONDITIONS_TO_COVER)).thenReturn(new Measure(CoreMetrics.SYSTEM_CONDITIONS_TO_COVER, (double) conditions));
    when(context.getMeasure(CoreMetrics.SYSTEM_UNCOVERED_CONDITIONS)).thenReturn(new Measure(CoreMetrics.SYSTEM_UNCOVERED_CONDITIONS, (double) uncoveredConditions));
    return context;
  }
  ...
  private static DecoratorContext mockNewContext(Measure newLines, Measure newUncoveredConditions, Measure newUncoveredLines, Measure newConditions) {
    DecoratorContext context = mock(DecoratorContext.class);
    when(context.getMeasure(CoreMetrics.NEW_SYSTEM_LINES_TO_COVER)).thenReturn(newLines);
    when(context.getMeasure(CoreMetrics.NEW_SYSTEM_UNCOVERED_LINES)).thenReturn(newUncoveredLines);
    when(context.getMeasure(CoreMetrics.NEW_SYSTEM_UNCOVERED_CONDITIONS)).thenReturn(newUncoveredConditions);
    when(context.getMeasure(CoreMetrics.NEW_SYSTEM_CONDITIONS_TO_COVER)).thenReturn(newConditions);
    return context;
  }
