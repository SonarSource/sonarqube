...
class AddGitlabSecurityManagerRolePermissionMappingTest {
  ...
  private final DataChange underTest = new AddGitlabSecurityManagerRolePermissionMapping(db.database());

  @Test
  void execute_shouldInsertSecurityManagerPermissions() throws SQLException {
    underTest.execute();

    List<Map<String, Object>> rows = db.select(
      "select sonarqube_permission from devops_perms_mapping where devops_platform = 'gitlab' and devops_platform_role = 'security manager'");
    assertThat(rows)
      .extracting(r -> r.get("sonarqube_permission"))
      .containsExactlyInAnyOrder("user", "codeviewer", "securityhotspotadmin", "issueadmin");
  }
  ...
  @Test
  void execute_whenSecurityManagerAlreadyExists_shouldNotInsert() throws SQLException {
    db.executeInsert("devops_perms_mapping",
      "uuid", "existing-uuid",
      "devops_platform", "gitlab",
      "devops_platform_role", "security manager",
