...
public class MigrationHistoryTableImplTest {
  ...
  @Test
  public void start_fails_with_RuntimeException_if_table_exists_without_the_version_column() throws SQLException {
    executeDdl("create table " + TABLE_SCHEMA_MIGRATIONS + " (foo varchar(10))");
    assertThat(dbTester.countRowsOfTable(TABLE_SCHEMA_MIGRATIONS)).isEqualTo(0);
    dbTester.assertColumnDefinition(TABLE_SCHEMA_MIGRATIONS, "foo", Types.VARCHAR, 10, true);

    expectedException.expect(RuntimeException.class);
    expectedException.expectMessage("Failed to create table " + TABLE_SCHEMA_MIGRATIONS);
    underTest.start();
  }
