...
public class CreateSlackSubscriptionsTable extends CreateTableChange {

  static final String SLACK_SUBSCRIPTIONS_TABLE_NAME = "slack_subscriptions";
  ...
  protected CreateSlackSubscriptionsTable(Database db) {
    super(db, SLACK_SUBSCRIPTIONS_TABLE_NAME);
  }
  ...
  public void execute(Context context, String tableName) throws SQLException {
    ...
    context.execute(new CreateTableBuilder(dialect, tableName)
      ...
      .addColumn(newVarcharColumnDefBuilder().setColumnName(COLUMN_CHANNEL_ID).setIsNullable(false).setLimit(255).build())
      .addColumn(newVarcharColumnDefBuilder().setColumnName(COLUMN_RESOURCE_TYPE).setIsNullable(false).setLimit(20).build())
      .addColumn(newVarcharColumnDefBuilder().setColumnName(COLUMN_RESOURCE_ID).setIsNullable(false).setLimit(40).build())
    ...
    context.execute(new CreateIndexBuilder(dialect)
      .setTable(tableName)
      .setName("ss_user_binding_uuid")
      .addColumn(COLUMN_USER_BINDING_UUID)
      .build());

    context.execute(new CreateIndexBuilder(dialect)
      .setTable(tableName)
      .setName("ss_slack_workspace_uuid")
      .addColumn(COLUMN_SLACK_WORKSPACE_UUID)
      .build());

    context.execute(new CreateIndexBuilder(dialect)
      .setTable(tableName)
      .setName("ss_resource_type_id")
      .addColumn(COLUMN_RESOURCE_TYPE)
      .addColumn(COLUMN_RESOURCE_ID)
      .build());
  }
