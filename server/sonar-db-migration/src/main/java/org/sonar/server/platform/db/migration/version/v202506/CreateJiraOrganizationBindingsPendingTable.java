...
public class CreateJiraOrganizationBindingsPendingTable extends CreateTableChange {

  static final String TABLE_NAME = "jira_org_bindings_pending";
  static final String PK_NAME = "pk_jira_org_bindings_pending";
  ...
  protected CreateJiraOrganizationBindingsPendingTable(Database db) {
    super(db, TABLE_NAME);
  }
  ...
  public void execute(Context context, String tableName) throws SQLException {
    context.execute(new CreateTableBuilder(getDialect(), tableName)
      ...
      .addColumn(newClobColumnDefBuilder().setColumnName(COLUMN_JIRA_ACCESS_TOKEN).setIsNullable(false).build())
      .addColumn(newBigIntegerColumnDefBuilder().setColumnName(COLUMN_JIRA_ACCESS_TOKEN_EXPIRES_AT).setIsNullable(false).build())
      .addColumn(newClobColumnDefBuilder().setColumnName(COLUMN_JIRA_REFRESH_TOKEN).setIsNullable(false).build())
      .addColumn(newBigIntegerColumnDefBuilder().setColumnName(COLUMN_JIRA_REFRESH_TOKEN_CREATED_AT).setIsNullable(false).build())
      .addColumn(newBigIntegerColumnDefBuilder().setColumnName(COLUMN_JIRA_REFRESH_TOKEN_UPDATED_AT).setIsNullable(false).build())
      .addColumn(newVarcharColumnDefBuilder().setColumnName(COLUMN_UPDATED_BY).setIsNullable(false).setLimit(40).build())
      .build());
  }
}
