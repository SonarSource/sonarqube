...
public class DuplicationMeasures {
  ...
  private final class DuplicationFormula implements Formula<DuplicationCounter> {
    ...
    private OptionalInt getNbLinesFromLocOrNcloc(CreateMeasureContext context) {
      ...
      if (lines.isPresent()) {
        return java.util.OptionalInt.of(lines.get().getIntValue());
      }
      ...
      if (nclocs.isPresent()) {
        ...
        return java.util.OptionalInt.of(commentLines.isPresent() ? (nbLines + commentLines.get().getIntValue()) : nbLines);
      }
