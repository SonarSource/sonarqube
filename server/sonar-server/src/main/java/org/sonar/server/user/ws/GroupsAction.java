...
public class GroupsAction implements BaseUsersWsAction {
  ...
  public void handle(Request request, Response response) throws Exception {
    ...
    UserDto user = dbClient.userDao().selectActiveUserByLogin(login);
    if (user == null) {
      throw new NotFoundException(String.format("Unable to find a user with login '%s'", login));
    }
    ...
    int total = dbClient.groupMembershipDao().countGroups(query, user.getId());

    Paging paging = Paging.create(pageSize, page, total);

    List<GroupMembershipDto> groups = dbClient.groupMembershipDao().selectGroups(query, user.getId(), paging.offset(), pageSize);
