...
public class GroupsAction implements UsersWsAction {
  ...
  private GroupsWsResponse doHandle(GroupsRequest request) {
    ...
    try (DbSession dbSession = dbClient.openSession(false)) {
      ...
      int defaultGroupId = checkFoundWithOptional(dbClient.organizationDao().getDefaultGroupId(dbSession, organization.getUuid()), "Organization '%s' has no default group",
        organization.getKey());
  ...
  private static GroupsWsResponse buildResponse(List<GroupMembershipDto> groups, int defaultGroupId, Paging paging) {
  ...
  private static Group toWsGroup(GroupMembershipDto group, int defaultGroupId) {
    Group.Builder groupBuilder = Group.newBuilder()
      .setId(group.getId())
      .setName(group.getName())
      .setSelected(group.getUserId() != null)
      .setDefault(defaultGroupId == group.getId());
