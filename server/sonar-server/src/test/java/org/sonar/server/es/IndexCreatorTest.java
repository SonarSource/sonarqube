...
public class IndexCreatorTest {
  ...
  private MetadataIndexDefinition metadataIndexDefinition = new MetadataIndexDefinition(new MapSettings());
  ...
  public void recreate_index_on_definition_changes() throws Exception {
    ...
    String id = "1";
    es.client().prepareIndex(fakeIndexType).setId(id).setSource(new FakeDoc().getFieldsWithoutId())
  ...
  public void do_not_recreate_index_on_unchanged_definition() throws Exception {
    ...
    // v1
    registry = new IndexDefinitions(new IndexDefinition[] {new FakeIndexDefinition()}, new MapSettings());
    registry.start();
    creator = new IndexCreator(es.client(), registry, metadataIndexDefinition, metadataIndex);
    creator.start();
    creator.stop();
