...
public class CreateTemplateActionTest {
  ...
  private final CreateTemplateAction underTest = new CreateTemplateAction(dbClient, userSession, mock(System2.class));

  @Test
  public void return_bad_request_when_template_is_created_concurrently() {
    configureTemplateCreation();
    when(permissionTemplateDao.selectByName(dbSession, "Finance"))
      .thenReturn(null, new PermissionTemplateDto().setName("Finance"));
    when(permissionTemplateDao.insert(any(), any())).thenThrow(new IllegalStateException("unique constraint violation"));
  ...
  @Test
  public void propagate_insert_failure_when_template_was_not_created_concurrently() {
    configureTemplateCreation();
    IllegalStateException failure = new IllegalStateException("database unavailable");
    when(permissionTemplateDao.selectByName(dbSession, "Finance")).thenReturn(null);
    when(permissionTemplateDao.insert(any(), any())).thenThrow(failure);
  ...
  private void configureTemplateCreation() {
    ...
    when(request.mandatoryParam("name")).thenReturn("Finance");
  }
}
