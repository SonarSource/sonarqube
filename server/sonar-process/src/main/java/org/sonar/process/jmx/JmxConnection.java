...
public class JmxConnection implements AutoCloseable {
  ...
  public SortedMap<String, Object> getSystemState() {
    ...
    props.put("Heap Committed (MB)", toMegaBytes(memory.getHeapMemoryUsage().getCommitted()));
    props.put("Heap Init (MB)", toMegaBytes(memory.getHeapMemoryUsage().getInit()));
    props.put("Heap Max (MB)", toMegaBytes(memory.getHeapMemoryUsage().getMax()));
    props.put("Heap Used (MB)", toMegaBytes(memory.getHeapMemoryUsage().getUsed()));
    props.put("Non Heap Committed (MB)", toMegaBytes(memory.getNonHeapMemoryUsage().getCommitted()));
    props.put("Non Heap Init (MB)", toMegaBytes(memory.getNonHeapMemoryUsage().getInit()));
    props.put("Non Heap Max (MB)", toMegaBytes(memory.getNonHeapMemoryUsage().getMax()));
    props.put("Non Heap Used (MB)", toMegaBytes(memory.getNonHeapMemoryUsage().getUsed()));
