...
class IssueBacklogTelemetryEventTest {
  ...
  void serialize_shouldUseSnakeCaseKeysAndNoTypeMetadataLeak() {
    IssueBacklogTelemetryEvent event = new IssueBacklogTelemetryEvent(
      ...
      "merge-branch-uuid",
      1_700_000_000_000L,
      List.of(new RuleCounts("java:S1234", 10, 5, 2, 1, 1)));
    ...
    assertThat(fieldNames(rule)).containsExactlyInAnyOrder(
      "plugin_rule_key", "open_count", "confirmed_count", "false_positive_count", "accepted_count", "in_sandbox_count");
  }
  ...
  void type_shouldExposeExpectedMetadata() {
    assertThat(IssueBacklogTelemetryEvent.TYPE.eventType()).isEqualTo("Analytics.Workflow.IssueBacklogAnalysisFinished");
