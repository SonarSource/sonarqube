...
import static org.sonar.core.config.PurgeConstants.DEFAULT_SCANNER_CACHE_RETENTION_DAYS;
...
public class AnalysisCacheCleaningSchedulerImpl implements AnalysisCacheCleaningScheduler {
  ...
  public AnalysisCacheCleaningSchedulerImpl(AnalysisCacheCleaningExecutorService executorService, DbClient dbClient, Configuration config) {
  ...
  @VisibleForTesting
  void clean() {
    int days = config.getInt(PurgeConstants.DAYS_BEFORE_DELETING_SCANNER_CACHE)
      .filter(d -> d > 0)
      .orElse(DEFAULT_SCANNER_CACHE_RETENTION_DAYS);
    try (var dbSession = dbClient.openSession(false)) {
      dbClient.scannerAnalysisCacheDao().cleanOlderThan(dbSession, days);
      dbSession.commit();
    }
  }
}
