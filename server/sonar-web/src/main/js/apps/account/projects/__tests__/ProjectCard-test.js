...
describe('My Account :: ProjectCard', () => {
  ...
  it('should render analysis date', () => {
    ...
    expect(output.find('.account-project-analysis').text()).to.contain('my_account.projects.analyzed_x');
  });
  ...
  it('should not render analysis date', () => {
    ...
    expect(output.find('.account-project-analysis').text()).to.contain('my_account.projects.never_analyzed');
  });

  it('should render quality gate status', () => {
    const project = { ...BASE, qualityGateStatus: 'ERROR' };
    const output = shallow(
        <ProjectCard project={project}/>
    );
    expect(output.find('.account-project-quality-gate').find(Level).prop('level'))
        .to.equal('ERROR');
  });
