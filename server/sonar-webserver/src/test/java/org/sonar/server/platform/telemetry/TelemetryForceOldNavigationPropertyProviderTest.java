...
class TelemetryForceOldNavigationPropertyProviderTest {
  private final Configuration config = mock();
  private final TelemetryForceOldNavigationPropertyProvider underTest = new TelemetryForceOldNavigationPropertyProvider(config);

  @ParameterizedTest
  @MethodSource("getValues")
  void getter_should_return_correct_values(Boolean value, Boolean expected) {
    if (value == null) {
      when(config.getBoolean(SONAR_UI_FORCE_OLD_NAVIGATION)).thenReturn(Optional.empty());
    } else {
      when(config.getBoolean(SONAR_UI_FORCE_OLD_NAVIGATION)).thenReturn(Optional.of(value));
    }

    assertEquals("force_old_navigation_enabled", underTest.getMetricKey());
