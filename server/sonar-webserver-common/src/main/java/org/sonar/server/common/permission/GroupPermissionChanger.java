...
public class GroupPermissionChanger implements GranteeTypeSpecificPermissionUpdater<GroupPermissionChange> {
  ...
  public Set<String> loadExistingEntityPermissions(DbSession dbSession, String organizationUuid, String uuidOfGrantee, @Nullable String entityUuid) {
    if (entityUuid != null) {
      return new HashSet<>(dbClient.groupPermissionDao().selectEntityPermissionsOfGroup(dbSession, organizationUuid, uuidOfGrantee, entityUuid));
    }
    return new HashSet<>(dbClient.groupPermissionDao().selectGlobalPermissionsOfGroup(dbSession, organizationUuid, uuidOfGrantee));
  }
  ...
  private void checkIfRemainingGlobalAdministrators(DbSession dbSession, GroupPermissionChange change) {
    ...
    if (GlobalPermission.ADMINISTER.getKey().equals(change.getPermission()) &&
      ...
      int remaining = dbClient.authorizationDao().countUsersWithGlobalPermissionExcludingGroup(dbSession, change.getOrganizationUuid(), OrganizationPermission.ADMINISTER.getKey(), groupUuid);
      checkRequest(remaining > 0, "Last group with permission '%s'. Permission cannot be removed.", OrganizationPermission.ADMINISTER.getKey());
    }
  }
