...
/**
 * A {@link DbSession} subclass which does not close the underlying session when {@link #close()} is called.
 * However, {@link #close()} will rollback the session if it is dirty
 * (i.e. if any insert, update or delete statement has been executed since the last commit or rollback).
 * In other words, the session is "reset" when closed, though not actually closed.
 */
