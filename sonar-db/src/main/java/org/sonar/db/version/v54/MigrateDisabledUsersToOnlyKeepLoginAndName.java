...
/**
 * Update all disabled users to only kept login and name
 */
public class MigrateDisabledUsersToOnlyKeepLoginAndName extends BaseDataChange {

  private final System2 system2;

  public MigrateDisabledUsersToOnlyKeepLoginAndName(Database db, System2 system2) {
    super(db);
  ...
  public void execute(Context context) throws SQLException {
    ...
    update.select("SELECT u.id FROM users u WHERE u.active=? " +
      "AND email IS NOT NULL " +
      "AND scm_accounts IS NOT NULL " +
      "AND external_identity IS NOT NULL " +
      "AND external_identity_provider IS NOT NULL " +
      "AND crypted_password IS NOT NULL " +
      "AND salt IS NOT NULL " +
      "AND remember_token IS NOT NULL " +
      "AND remember_token_expires_at IS NOT NULL "
    ).setBoolean(1, false);
    update.update("UPDATE users SET email=null, scm_accounts=null, " +
      "external_identity=null, external_identity_provider=null, " +
      "crypted_password=null, salt=null, " +
